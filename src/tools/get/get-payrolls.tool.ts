...
const getPayrollsTool = CreateTool(
    ...
    Also includes status metadata such as cancellation eligibility, submission blockers, credit blockers, and reasons for any changes in processing speed.`,
  {
    ...
    processingStatuses: z
      .array(z.enum(['processed', 'unprocessed']))
      .optional()
      .describe('Processing statuses to filter payrolls'),
    payrollTypes: z
      .array(z.enum(['regular', 'off_cycle']))
      .optional()
      .describe('Payroll types to filter payrolls'),
  ...
  async (args) => {
    ...
    const payrollsText = payrollsList.payrollList
      ?.map((payroll) => generatePayrollText(payroll))
      .join('\n\n');
    return {
      content: [
        {
